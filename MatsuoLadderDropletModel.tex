...
\title{\textbf{A Mechanistic Kinetic Model of Peptide Oligomerization and Droplet Formation in
the Matsuo--Kurihara System}}
...
\begin{center}
\small Cite as: \href{https://doi.org/10.5281/zenodo.22717387}{DOI 10.5281/zenodo.22717387}
\end{center}
...
identifying an effective DTT reaction order of $p\approx2.37$ instead.
...
image. A likely error in the Source Data file itself was found and corrected (not confirmed
by the original authors, but supported by an exact match described below):
\textbf{the Fig.~3d column headers for $25$~mM and $13$~mM DTT appear to be swapped relative
to the data they actually label.} The column headed ``$25$'' decays \emph{more slowly} than
the column headed ``$13$'', which is chemically backwards --- more DTT should reduce
...
slower-decaying curve. This note uses the assignment supported by that exact match throughout;
the true $13$~mM curve
...
$p$ & effective DTT reaction order & fit, jointly \emph{only} & $2.37$\\
...
\end{tabular}
\end{center}
One shared mechanism fits all three curves well --- the DTT-reduction step $R_0$ has an
effective DTT reaction order of $p=2.37$ within this model, well above the naive first-order
guess. (This is an effective, fitted exponent, not a claim about the elementary reaction's true
molecularity.) This is the model's
...
already validated Stage~1's effective DTT reaction order. A successful prediction completes the
model.
...
Curve family & Richards (unnamed as such) & drifting-target $\times$ Avrami response (\S\ref{sec:model-droplet})\\[2pt]
...
\textbf{2. An effective DTT reaction order.} This note determines it
directly: $p\approx2.37$, well above the first-order guess a naive mass-action
model would start from (\S\ref{sec:ladder-fit}) --- a concrete, checkable claim about the
fitted rate law's DTT dependence, made possible by fitting a rate law to the oligomerization
data in the first place, though not by itself a claim about the elementary reaction's true
molecularity.
...
identifies the effective reaction order.
...
The predictive validation established for Stage~1 does not yet extend to Stage~2 itself (data
limitations, see again \S\ref{sec:droplet-held-out}--\S\ref{sec:structural-limit}).
